...
public class RankedUpdateFunction implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent event, Context context) {
        ...
        try {
            ...
            rankRecord.getRank()rankRecord.getRank() + getRankDelta(helper.getParam("status")));
            userId = ddbUtils.addRankRecord(rankRecord);

        } catch (JsonProcessingException e) {
            logger.log(e.getMessage());
            return response.withStatusCode(400)
                    .withBody("{\"message\": \"Body incorrect.\"}");
        }
        return response.withStatusCode(200)
                .withBody(String.format("{\"message\": \"Succesfully update rank for user %s\"}", userId));
    }
